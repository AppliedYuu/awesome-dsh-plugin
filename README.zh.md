...
- [AppliedYuu/dsh-WallpaperAndCost](https://github.com/AppliedYuu/dsh-WallpaperAndCost) — 壁纸定制（原图不压缩、裁剪编辑器、左/中/右分区不透明度、MP4 动态壁纸）加 Steam 创意工坊壁纸提取（内置 scripts/extract_scene_pkg.py 解析 scene.pkg/.tex，支持预览与下载），以及 DeepSeek 余额与用量小组件，支持书签一键获取平台 Token。
